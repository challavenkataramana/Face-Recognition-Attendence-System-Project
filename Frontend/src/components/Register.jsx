...
const Register = () => {
  ...
  const startCamera = async () => {
    try {
      ...
      if (videoRef.current) {
        ...
        setMessage({ text: "Camera ready. Position your face in the frame.", type: "info" });
      }
    } catch (err) {
  ...
  const handleCaptureClick = async () => {
    if (!name.trim()) {
      setMessage({ text: "Please enter your name before capturing.", type: "error" });
      return;
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage({ text: registerRes.data.message || "Registration successful!", type: "success" });

      if (registerRes.data.type=="success") {
        try {
          ...
          setMessage({ 
            text: "Registration complete but encoding failed. " + 
      ...
      setMessage({ 
        text: err.response?.data?.error || "Error during registration. Please try again.", 
        type: "error" 
      });
    } finally {
      setIsLoading(false);
                ...
                {!cameraActive && (
                  <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 rounded-lg">
                    <div className="text-white text-center p-4">
                      <svg className="w-12 h-12 mx-auto mb-2 animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              ...
              <div className="flex space-x-3 mt-4">
                <button
                  ...
                  className={`flex-1 py-2 px-4 rounded-lg font-medium transition
                    ${(!cameraActive || isLoading) ? 'bg-gray-300 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700 text-white'}
                    flex items-center justify-center`}
                >
